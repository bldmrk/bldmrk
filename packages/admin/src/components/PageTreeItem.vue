...
<template>
  <li>
    ...
    >
      ...
      <span class="text-sm text-gray-900 flex-1 truncate">{{ node.data.title }}</span>

      <span
        :class="['text-xs px-1.5 py-0.5 rounded', node.data.published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500']"
      >{{ node.data.published ? 'pub' : 'draft' }}</span>
